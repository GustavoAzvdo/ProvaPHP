...
class VendasModels
{
    ...
    public static function produtoFoiVendido(int $ID_produto): bool
    {
        try {
            $con = Database::conectar();
            $sql = "SELECT COUNT(*) FROM vendas WHERE ID_produto = :ID_produto";
            $stmt = $con->prepare($sql);
            $stmt->bindParam(':ID_produto', $ID_produto);
            $stmt->execute();
            return $stmt->fetchColumn() > 0;
        } catch (PDOException $e) {
            error_log("Erro ao verificar vendas do produto: " . $e->getMessage());
            return true;
        }
    }
}
